...
def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status from tabAttendance where employee in (select name from `tabEmployee` where cash_employee_custom = 0) and docstatus = 1 %s order by employee, attendance_date""" %
